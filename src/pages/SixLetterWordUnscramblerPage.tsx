...
export default function SixLetterWordUnscramblerPage({ onNavigate }: SixLetterWordUnscramblerPageProps) {
  ...
  useEffect(() => {
    ...
    if (queryLetters) {
      const cleanLetters = queryLetters.replace(/[^a-zA-Z?*]/g, '').slice(0, 6);
      if (cleanLetters) {
  ...
  return (
    <main id="six-letter-page" className="flex-1 w-full max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
      {/* Hero / Header */}
      <div className="text-center mb-8 sm:mb-10 space-y-3">
        <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-indigo-50 text-indigo-700 border border-indigo-200/60 mb-1">
          ...
          <span>6-Letter Anagram & Word Solver</span>
        </div>
        <h1
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-start">
        {/* Left Column: 6 Letter Unscrambler Tool */}
