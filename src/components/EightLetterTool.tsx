...
interface EightLetterToolProps {
...
export default function EightLetterTool({
  onSearch,
  onClear,
  dictStatus,
  initialLetters = '',
  initialFilters,
}: EightLetterToolProps) {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Filter to valid characters (A-Z, ?) and allow a maximum of 8 characters
  ...
  const executeSearch = (lettersToSearch: string, searchFilters: FilterState) => {
    ...
    if (!rawClean) {
      setValidationError('Please enter up to 8 letters or wildcards (?).');
      return;
    }

    if (rawClean.length > 8) {
      setValidationError('Please enter up to 8 letters.');
      return;
    }
  ...
  return (
    <div id="eight-letter-tool-container" className="w-full flex flex-col gap-6">
      {/* Main Tool Card */}
      <div
        id="eight-letter-tool-card"
      ...
      >
        ...
        <form id="eight-letter-form" onSubmit={handleSubmit} className="w-full space-y-5">
          <div className="space-y-2">
            {/* Input Label */}
            <div className="flex items-center justify-between">
              <label
                id="eight-letters-input-label"
                htmlFor="eight-letters-input"
                className="text-xs font-bold uppercase tracking-wider text-slate-500 flex items-center gap-1.5"
              >
                <span>Enter up to 8 letters</span>
                <span
              ...
              <span id="eight-letters-counter" className="text-xs text-slate-400 font-medium">
                {inputVal.length} / 8
              </span>
            </div>
            ...
            <div className="relative">
              <input
                ...
                placeholder="Enter up to 8 letters, e.g. NOTEBOOK or TREASURE"
                aria-describedby="eight-letters-helper-text"
              ...
              {inputVal.length > 0 && (
                <button
                  id="eight-inline-clear-btn"
                  type="button"
                  onClick={handleClear}
                  className="absolute right-3 top-1/2 -translate-y-1/2 rounded p-1 text-slate-400 hover:bg-slate-200 hover:text-slate-700 transition-colors cursor-pointer"
            ...
            {validationError && (
              <div
                id="eight-letters-validation-error"
                role="alert"
            ...
            {/* Helper Text */}
            <p id="eight-letters-helper-text" className="text-xs text-slate-500 leading-relaxed">
              Use <code className="font-mono bg-slate-100 px-1 py-0.5 rounded text-indigo-600 font-bold border border-slate-200">?</code> or <code className="font-mono bg-slate-100 px-1 py-0.5 rounded text-indigo-600 font-bold border border-slate-200">*</code> for wildcard tiles (up to 3). Defaults to 8-letter words.
            </p>
          </div>

          {/* Tactile Letter Tiles Preview (Up to 8 slots) */}
          <div id="eight-tiles-preview-container" className="space-y-1.5">
            <span className="text-[11px] font-bold uppercase tracking-wider text-slate-400 block">
              Tile Rack Preview ({previewChars.length}/8)
            </span>
          ...
          <div id="eight-letter-filters-box" className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-4">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold uppercase tracking-wider text-slate-600">
                Search Filters
              </span>
              <span className="text-[11px] text-slate-400">
                Word length set to 8 letters
              </span>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div className="space-y-1">
                <label
                  htmlFor="eight-sort-by-select"
                  className="text-xs font-medium text-slate-700 block"
                >
                  Sort Order
                </label>
                <select
                  id="eight-sort-by-select"
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-1">
            <button
              id="eight-unscramble-submit-btn"
              type="submit"
            ...
            >
              {dictStatus && !dictStatus.loaded ? (
                ...
                <>
                  ...
                  <span>Unscramble 8 Letter Words</span>
      ...
      {recentSearches.length > 0 && (
        <section
          id="eight-recent-searches-section"
          aria-labelledby="eight-recent-searches-heading"
        ...
        >
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <h2
                id="eight-recent-searches-heading"
