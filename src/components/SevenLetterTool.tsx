...
import { Search, X, CornerDownLeft, AlertCircle, Loader2, History, Trash2, HelpCircle, Sparkles } from 'lucide-react';
...
interface SevenLetterToolProps {
...
export default function SevenLetterTool({
  onSearch,
  onClear,
  dictStatus,
  initialLetters = '',
  initialFilters,
}: SevenLetterToolProps) {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Filter to valid characters (A-Z, ?) and allow a maximum of 7 characters
  ...
  const executeSearch = (lettersToSearch: string, searchFilters: FilterState) => {
    ...
    if (!rawClean) {
      setValidationError('Please enter up to 7 letters or wildcards (?).');
      return;
    }

    if (rawClean.length > 7) {
      setValidationError('Please enter up to 7 letters.');
      return;
    }
  ...
  return (
    <div id="seven-letter-tool-container" className="w-full flex flex-col gap-6">
      {/* Main Tool Card */}
      <div
        id="seven-letter-tool-card"
      ...
      >
        ...
        <form id="seven-letter-form" onSubmit={handleSubmit} className="w-full space-y-5">
          <div className="space-y-2">
            {/* Input Label */}
            <div className="flex items-center justify-between">
              <label
                id="seven-letters-input-label"
                htmlFor="seven-letters-input"
                className="text-xs font-bold uppercase tracking-wider text-slate-500 flex items-center gap-1.5"
              >
                <span>Enter up to 7 letters</span>
                <span
              ...
              <span id="seven-letters-counter" className="text-xs text-slate-400 font-medium">
                {inputVal.length} / 7
              </span>
            </div>
            ...
            <div className="relative">
              <input
                ...
                placeholder="Enter up to 7 letters, e.g. ARETSL? or START??"
                aria-describedby="seven-letters-helper-text"
            ...
            {validationError && (
              <div
                id="seven-letters-validation-error"
                role="alert"
            ...
            {/* Helper Text */}
            <p id="seven-letters-helper-text" className="text-xs text-slate-500 leading-relaxed">
              Use <code className="font-mono bg-slate-100 px-1 py-0.5 rounded text-indigo-600 font-bold border border-slate-200">?</code> or <code className="font-mono bg-slate-100 px-1 py-0.5 rounded text-indigo-600 font-bold border border-slate-200">*</code> for wildcard tiles (up to 3). Defaults to 7-letter words.
            </p>
          </div>

          {/* Tactile Letter Tiles Preview (Up to 7 slots) */}
          <div id="seven-tiles-preview-container" className="space-y-1.5">
            <span className="text-[11px] font-bold uppercase tracking-wider text-slate-400 block">
              Tile Rack Preview ({previewChars.length}/7)
            </span>
          ...
          <div id="seven-letter-filters-box" className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-4">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold uppercase tracking-wider text-slate-600">
                Search Filters
              </span>
              <span className="text-[11px] text-slate-400">
                Word length set to 7 letters
              </span>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div className="space-y-1">
                <label
                  htmlFor="seven-sort-by-select"
                  className="text-xs font-medium text-slate-700 block"
                >
                  Sort Order
                </label>
                <select
                  id="seven-sort-by-select"
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-1">
            <button
              id="seven-unscramble-submit-btn"
              type="submit"
            ...
            >
              {dictStatus && !dictStatus.loaded ? (
                ...
                <>
                  ...
                  <span>Unscramble 7 Letter Words</span>
      ...
      {recentSearches.length > 0 && (
        <section
          id="seven-recent-searches-section"
          aria-labelledby="seven-recent-searches-heading"
        ...
        >
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <h2
                id="seven-recent-searches-heading"
